...
export default async function handler(req, res) {
   ...
   *
   * we pass that exact token to SerpApi.
   *
   * IMPORTANT:
   * The token is NOT regenerated by the backend.
   * SerpApi controls the pagination token.
   * =======================================================
   */
   ...
   * It helps verify that pagination requests
   * are actually reaching the backend.
   * -------------------------------------------------------
   */

  console.log(
    "Bokkara Hotels Request:",
    {
      destination: searchQuery,

      has_next_page_token:
        Boolean(next_page_token),

      next_page_token_length:
        next_page_token
          ? String(next_page_token).length
          : 0,
  ...
  try {
     ...
     * =======================================================
     *
     * SerpApi normally returns:
     *
     * data.next_page_token
     *
     * But we also check pagination.next_page_token
     * so the backend remains compatible with alternate
     * response structures.
     * =======================================================
     */
    ...
    const nextToken =
      (
        ...
        ? data.next_page_token.trim()
        : (
            typeof data.pagination?.next_page_token === "string" &&
            data.pagination.next_page_token.trim()
          )
            ? data.pagination.next_page_token.trim()
            : null;
    ...
    return res.status(200).json({
       ...
       * =====================================================
       *
       * Shopify should save this token and send it back
